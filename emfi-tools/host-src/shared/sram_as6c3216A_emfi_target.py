...
# Notes: actual chip size is 12.2 mm x 18.4 mm.
...
class sram_emfi_target:
    ...
    def examine_target_low_resolution(self):
        ...
        result = self.examine_target()
        errorlist = result['errorlist']
        
        # reduce error reporting to a simple single value.
        for i in range(len(errorlist)):
            if errorlist[i] > 0:
                retValue = self.EXAMINE_RET_CODE.SRAM_BIT_ERRORS
                break; # return early. Only need is to find one error 

        return retValue
    
    def reload_target(self):
        # return self.__reinitialize_target
        # Always reload target - it is quick and allows for full power EMP charge.
        return True    
